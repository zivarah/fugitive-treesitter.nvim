...
local function colored_parts_on(buf, row)
  local line = vim.api.nvim_buf_get_lines(buf, row, (row + 1), false)[1]
  local tbl_26_ = {}
  local i_27_ = 0
  for _, mark in ipairs(marks(buf)) do
    local val_28_
    if ((row == mark.row) and mark["hl-group"] and not capture_3f(mark)) then
      val_28_ = {text = string.sub(line, (mark.col + 1), mark["end-col"]), ["hl-group"] = mark["hl-group"]}
    else
      val_28_ = nil
    end
    if (nil ~= val_28_) then
...
local function _12_()
  ...
  it("colors added and removed lines but not context lines", _13_)
  local function _14_()
    local buf = diff_buffer("git", one_file)
  ...
  it("ends the line background where the line ends", _14_)
  local function _15_()
  ...
  it("colors a line that holds nothing but its marker", _15_)
  local function _16_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
    return assert.same({{text = "-local timeout = 30", ["hl-group"] = highlight["delete-group"]}}, colored_parts_on(buf, 5))
  end
  it("colors an ordinary diff line with one mark from its marker on", _16_)
  local function _17_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("places treesitter captures shifted past the diff marker", _17_)
  local function _18_()
  ...
  it("highlights a fugitive status buffer", _18_)
  local function _19_()
  ...
  it("highlights a combined diff in a status buffer", _19_)
  local function _20_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("replaces the highlights rather than stacking them", _20_)
  local function _21_()
  ...
  it("colors past an empty context line", _21_)
  local function _22_()
  ...
  it("still colors lines when the language has no parser", _22_)
  local function _23_()
    local buf = diff_buffer("git", {"commit abc123", "Author: Nobody"})
    render.buffer(buf)
    return assert.equals(0, #marks(buf))
  end
  return it("does nothing to a buffer with no hunk", _23_)
end
describe("buffer", _12_)
local function _24_()
  local function _25_()
  ...
  it("keeps the other side's lines out of the parse", _25_)
  local function _26_()
  ...
  it("parses each side of a rename with its own parser", _26_)
  local function _27_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("colors a context line once, not once per side", _27_)
  local function _28_()
  ...
  it("colors an injected language inside a hunk", _28_)
  local function _29_()
  ...
  return it("colors a side whose file has no parser", _29_)
end
describe("per-side parsing", _24_)
...
local function _30_()
  ...
  it("colors the lines by the marker of the patch", _31_)
  local function _32_()
    local buf = diff_buffer("git", one_pair)
    render.buffer(buf)
    return assert.same({{text = "-", ["hl-group"] = highlight["delete-group"]}, {text = "+local timeout = 45", ["hl-group"] = highlight["add-dim-group"]}}, colored_parts_on(buf, 5))
  end
  it("colors the marker of the series apart from the rest of the line", _32_)
  local function _33_()
    local buf = diff_buffer("git", one_pair)
    render.buffer(buf)
    return assert.same({{text = "-local timeout = 30", ["hl-group"] = highlight["delete-group"]}}, colored_parts_on(buf, 4))
  end
  it("leaves the marker of a line that both series hold uncolored", _33_)
  local function _34_()
    local buf = diff_buffer("git", one_pair)
    render.buffer(buf)
    return assert.same({{text = "1:  1111111", ["hl-group"] = highlight["commit-delete-group"]}, {text = "!", ["hl-group"] = highlight["commit-group"]}, {text = "1:  2222222", ["hl-group"] = highlight["commit-add-group"]}}, colored_parts_on(buf, 0))
  end
  it("colors the entry that pairs two commits", _34_)
  local function _35_()
    local buf = diff_buffer("git", one_pair)
    render.buffer(buf)
    assert.same({{text = "a.lua", ["hl-group"] = highlight["file-group"]}}, colored_parts_on(buf, 1))
    return assert.same({{text = "@@", ["hl-group"] = highlight["patch-hunk-group"]}}, colored_parts_on(buf, 2))
  end
  it("colors the hunk header of a patch and the heading above it", _35_)
  local function _36_()
    local buf = diff_buffer("git", {" 1:  1111111 ! 1:  2222222 fix: x", "    @@ Commit message", "    -    fix: old subject", "    +    fix: new subject"})
    render.buffer(buf)
    assert.same({}, line_groups(buf))
    assert.same({{text = "-", ["hl-group"] = highlight["delete-group"]}}, colored_parts_on(buf, 2))
    return assert.same({{text = "+", ["hl-group"] = highlight["add-group"]}}, colored_parts_on(buf, 3))
  end
  it("colors the marker of a line that no hunk covers", _36_)
  local function _37_()
  ...
  it("dims only the lines that the earlier series alone holds", _37_)
  local function _38_()
    local buf = diff_buffer("git", one_pair)
    render.buffer(buf)
  ...
  it("parses each series apart from the other", _38_)
  local function _39_()
    local buf = diff_buffer("git", one_pair)
    render.buffer(buf)
    assert.equals("@number", captures_on(buf, 4)["30"])
  ...
  return it("colors a line that both series hold once", _40_)
end
describe("range-diff output", _30_)
local function _41_()
  local function _42_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  return it("removes every highlight the plugin placed", _42_)
end
return describe("clear", _41_)
