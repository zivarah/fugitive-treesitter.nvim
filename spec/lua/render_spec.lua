...
local function _9_()
  ...
  it("still colors lines when the language has no parser", _15_)
  local function _16_()
    local buf = diff_buffer("git", {"commit abc123", "Author: Nobody"})
    render.buffer(buf)
    return assert.equals(0, #marks(buf))
  end
  return it("does nothing to a buffer with no hunk", _16_)
end
describe("buffer", _9_)
local function _17_()
  local function _18_()
  ...
  it("keeps the other side's lines out of the parse", _18_)
  local function _19_()
  ...
  it("parses each side of a rename with its own parser", _19_)
  local function _20_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("colors a context line once, not once per side", _20_)
  local function _21_()
  ...
  it("colors an injected language inside a hunk", _21_)
  local function _22_()
  ...
  return it("colors a side whose file has no parser", _22_)
end
describe("per-side parsing", _17_)
local function _23_()
  local function _24_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  return it("removes every highlight the plugin placed", _24_)
end
return describe("clear", _23_)
