...
local function marks(buf)
  ...
  for _, mark in ipairs(vim.api.nvim_buf_get_extmarks(buf, ns, 0, -1, {details = true})) do
    ...
    do
      ...
      val_28_ = {row = row, col = col, ["end-col"] = details.end_col, ["hl-group"] = details.hl_group, ["hl-eol"] = details.hl_eol, ["line-hl-group"] = details.line_hl_group}
    end
    if (nil ~= val_28_) then
      i_27_ = (i_27_ + 1)
...
local function captures_on(buf, row)
  ...
  for _, mark in ipairs(marks(buf)) do
    local k_22_, v_23_
    if ((row == mark.row) and mark["hl-group"]) then
...
local function _9_()
  ...
  it("colors added and removed lines but not context lines", _10_)
  local function _11_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
    for _, mark in ipairs(marks(buf)) do
    ...
    return assert.equals(highlight["delete-group"], line_groups(buf)[5])
  end
  it("colors a line with hl_eol rather than with a line_hl_group", _11_)
  local function _12_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("places treesitter captures shifted past the diff marker", _12_)
  local function _13_()
  ...
  it("highlights a fugitive status buffer", _13_)
  local function _14_()
  ...
  it("highlights a combined diff in a status buffer", _14_)
  local function _15_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("replaces the highlights rather than stacking them", _15_)
  local function _16_()
  ...
  it("colors past an empty context line", _16_)
  local function _17_()
  ...
  it("still colors lines when the language has no parser", _17_)
  local function _18_()
    local buf = diff_buffer("git", {"commit abc123", "Author: Nobody"})
    render.buffer(buf)
    return assert.equals(0, #marks(buf))
  end
  return it("does nothing to a buffer with no hunk", _18_)
end
describe("buffer", _9_)
local function _19_()
  local function _20_()
  ...
  it("keeps the other side's lines out of the parse", _20_)
  local function _21_()
  ...
  it("parses each side of a rename with its own parser", _21_)
  local function _22_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("colors a context line once, not once per side", _22_)
  local function _23_()
  ...
  it("colors an injected language inside a hunk", _23_)
  local function _24_()
  ...
  return it("colors a side whose file has no parser", _24_)
end
describe("per-side parsing", _19_)
local function _25_()
  local function _26_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  return it("removes every highlight the plugin placed", _26_)
end
return describe("clear", _25_)
