...
local function _9_()
  ...
  it("replaces the highlights rather than stacking them", _14_)
  local function _15_()
  ...
  it("colors past an empty context line", _15_)
  local function _16_()
  ...
  it("still colors lines when the language has no parser", _16_)
  local function _17_()
    local buf = diff_buffer("git", {"commit abc123", "Author: Nobody"})
    render.buffer(buf)
    return assert.equals(0, #marks(buf))
  end
  return it("does nothing to a buffer with no hunk", _17_)
end
describe("buffer", _9_)
local function _18_()
  local function _19_()
  ...
  it("keeps the other side's lines out of the parse", _19_)
  local function _20_()
  ...
  it("parses each side of a rename with its own parser", _20_)
  local function _21_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  it("colors a context line once, not once per side", _21_)
  local function _22_()
  ...
  it("colors an injected language inside a hunk", _22_)
  local function _23_()
  ...
  return it("colors a side whose file has no parser", _23_)
end
describe("per-side parsing", _18_)
local function _24_()
  local function _25_()
    local buf = diff_buffer("git", one_file)
    render.buffer(buf)
  ...
  return it("removes every highlight the plugin placed", _25_)
end
return describe("clear", _24_)
