...
local function _9_()
  local function _10_()
    ...
    return assert.same({first = 5, last = 9, ["marker-width"] = 1, ["text-col"] = 1, ["old-path"] = "lua/foo.lua", ["new-path"] = "lua/foo.lua"}, regions[1])
  end
  ...
  it("strips a one-letter directory prefix", _13_)
  local function _14_()
  ...
  it("uses the removed path for both sides of a deleted file", _14_)
  local function _15_()
  ...
  it("uses the added path for both sides of a new file", _15_)
  local function _16_()
  ...
  it("keeps both paths of a rename", _16_)
  local function _17_()
  ...
  it("keeps a path that contains a slash-prefixed directory", _17_)
  local function _18_()
  ...
  it("finds one region per file", _18_)
  local function _19_()
  ...
  it("finds both hunks of one file", _19_)
  local function _20_()
  ...
  it("reads the marker width of a combined diff", _20_)
  local function _21_()
    return assert.same({}, git({"@@ -1 +1 @@", "-local a = 1", "+local a = 2"}))
  end
  it("gives nothing for a hunk with no file header", _21_)
  local function _22_()
    return assert.same({}, git({"diff --git a/a.lua b/a.lua", "--- a/a.lua", "+++ b/a.lua", "@@ -0,0 +0,0 @@"}))
  end
  it("gives nothing for a header with an empty body", _22_)
  local function _23_()
    assert.same({}, git({}))
    return assert.same({}, git({"commit abc123", "Author: Someone"}))
  end
  return it("gives nothing for a buffer with no hunk", _23_)
end
...
local function _24_()
  local function _25_()
    ...
    assert.equals(1, #regions)
    return assert.same({first = 5, last = 7, ["marker-width"] = 1, ["text-col"] = 1, ["old-path"] = "lua/foo.lua", ["new-path"] = "lua/foo.lua"}, regions[1])
  end
  it("takes the path from a status entry", _25_)
  local function _26_()
    local regions = status({"Staged (1)", "R old.js -> new.ts", "@@ -1 +1 @@", "-var a = 1", "+const a: number = 1"})
    assert.equals("old.js", regions[1]["old-path"])
    return assert.equals("new.ts", regions[1]["new-path"])
  end
  it("splits both paths of a rename entry", _26_)
  local function _27_()
    local regions = status({"Unstaged (1)", "M a.lua", "@@ -1 +1 @@", "-local a = 1", "+local a = 2"})
    assert.equals("a.lua", regions[1]["old-path"])
    return assert.equals("a.lua", regions[1]["new-path"])
  end
  it("uses one path for both sides of a plain entry", _27_)
  local function _28_()
    local regions = status({"Untracked (1)", "? new.lua", "@@ -0,0 +1 @@", "+local a = 1"})
    return assert.equals("new.lua", regions[1]["new-path"])
  end
  it("handles an untracked entry", _28_)
  local function _29_()
    return assert.same({}, status({"--- a/a.lua", "+++ b/a.lua", "@@ -1 +1 @@", "-local a = 1", "+local a = 2"}))
  end
  return it("does not read a git-format header as a path", _29_)
end
return describe("regions in a fugitive status buffer", _24_)
